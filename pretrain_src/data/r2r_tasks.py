...
class MimImageDataset(MrcDataset):
    def __init__(self, nav_db, tok, mask_prob, dvae_filter, distill=False):
        self.nav_db = nav_db
        self.tok = tok
        self.mask_prob = mask_prob
        self.dvae_filter = dvae_filter
    ...
    def __getitem__(self, i):
        ...
        hist_mrc_masks = _get_img_mask(
            self.mask_prob, inputs["hist_images_dvae"].shape[0]
        )
        ...
        if self.dvae_filter:
            output['dvae_filter'] = torch.from_numpy(np.array(self.dvae_filter))
        else:
            output['dvae_filter'] = None

        return output


def mim_image_collate(inputs):
    ...
    batch["hist_images_dvae"] = pad_tensors(
        batch["hist_images_dvae"], lens=batch["hist_lens"], pad=0
    )
    ...
    batch["dvae_filter"] = batch["dvae_filter"][0]

    return batch


############### Single-step Predict Action As Image Generation ###############
class ApwigImageDataset(Dataset):
    def __init__(self, nav_db, tok, dvae_filter):
        '''Single Step Action Regression'''
        self.nav_db = nav_db
        self.tok = tok
        self.dvae_filter = dvae_filter

        self.random_kill_v = 0
        self.random_kill_a = 0
    ...
    def __getitem__(self, i):
        ...
        inputs = self.nav_db.get_input(i_traj, j_instr, t_cur,
                                       return_ob=True, return_ob_action=True,
                                       return_hist_img_probs=False, return_ob_progress=False, return_ob_image=True)
        ...
        output['ob_action_image'] = inputs['ob_action_image'].unsqueeze(0)
...
def apwig_collate(inputs):
    ...
    batch['ob_action_image'] = torch.cat(batch['ob_action_image'], 0)
    batch["dvae_filter"] = batch["dvae_filter"][0]
...
class MppDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        inputs = self.nav_db.get_input(i_traj, j_instr, t_cur,
            return_ob=True, return_ob_action=True,
            return_hist_img_probs=False, return_ob_progress=False, return_ob_pano_images=True, pano_mask_prob=self.mask_prob)
        ...
        output['ob_pano_images'] = torch.from_numpy(inputs['ob_pano_images']).unsqueeze(0)

        output["ob_mpp_masks"] = inputs['ob_pano_mask']

        if not self.distill:
            output['ob_img_fts'] = _mask_img_feat(
            torch.from_numpy(inputs['ob_img_fts']),
            inputs['ob_pano_mask'])
...
def mpp_collate(inputs):
    ...
    batch['ob_pano_images'] = torch.cat(batch['ob_pano_images'], 0)

    batch["dvae_filter"] = batch["dvae_filter"][0]

    batch["ob_mpp_masks"] = pad_sequence(
        batch["ob_mpp_masks"], batch_first=True, padding_value=0
    )
